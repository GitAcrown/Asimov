...
class Downloader:
    ...
    async def _repo_add(self, ctx, repo_name: str, repo_url: str):
        """Ajoute un repo à la liste.

        Attention : Cette action peut être dangereuse si la source est inconnue."""
        await self.bot.say("Tapez 'Je confirme' pour accepter d'ajouter un module d'une source externe au bot.\nCette action vous empêchera d'obtenir un support en cas de problèmes.\nN'installez ce repo seulement si vous êtes certain qu'il ne causera pas de dégats.")
        answer = await self.bot.wait_for_message(timeout=15,
                                                 author=ctx.message.author)
        if answer is None:
            await self.bot.say('Repo non ajouté.')
            return
        elif "je confirme" not in answer.content.lower():
            await self.bot.say('Repo non ajouté.')
            return
        ...
        try:
            ...
            await self.bot.say("Cet URL est invalide.")
        ...
        await self.bot.say("Repo '{}' ajouté.".format(repo_name))

    @repo.command(name="remove")
    async def _repo_del(self, repo_name: str):
        """Retire un repo de la liste. NE RETIRE PAS LES MODULES INSTALLES."""
        if repo_name not in self.repos:
            await self.bot.say("Ce repo n'existe pas.")
            return
        del self.repos[repo_name]
        #shutil.rmtree(os.path.join(self.path, repo_name))
        self.save_repos()
        await self.bot.say("Repo '{}' retiré.".format(repo_name))

    @cog.command(name="list")
    async def _send_list(self, repo_name=None):
        """Liste les modules installable."""
        retlist = []
        if repo_name and repo_name in self.repos:
            msg = "Modules disponibles:\n"
    ...
    async def info(self, repo_name: str, cog: str=None):
        """Affiche des informations à propos d'un module."""
        if cog is not None:
            ...
            if cog in cogs:
                ...
                if data:
                    ...
                    await self.bot.say("Ce module ne possède pas d'informations.")
            else:
                await self.bot.say("Ce module n'existe pas.")
        else:
            data = self.get_info_data(repo_name)
            if data is None:
                await self.bot.say("Le module n'existe pas ou ses informations ne sont pas disponibles.")
                return
    ...
    async def update(self, ctx):
        """Met à jour les modules installés."""
        ...
        base_msg = "Téléchargement des mises à jour... "
        status = ' %d/%d repos MAJ' % (tasknum, num_repos)
        ...
        if not any(self.repos[repo][cog]['INSTALLED'] for
            ...
            status += ' Aucune mise à jour disponible. '
        ...
        if error_repos:
            status += '\nLes modules suivants n\'ont pas étés mis à jour: '
        ...
        await self.bot.say("Modules installés. Voulez-vous les recharger ? (yes/no)")
        answer = await self.bot.wait_for_message(timeout=15,
                                                 author=ctx.message.author)
        if answer is None:
            await self.bot.say("Vous pourrez le faire plus tard avec"
            ...
            msg = 'Done.'
            if update_list:
                msg += " Ces modules ont été rechargés: "\
                    + ', '.join(update_list) + "\n"
            if fail_list:
                msg += " Ces modules n'ont pas pu être rechargés: "\
                    + ', '.join(fail_list)
            await self.bot.say(msg)
    ...
    async def uninstall(self, ctx, repo_name, cog):
        ...
        if cog not in self.repos[repo_name]:
            await self.bot.say("Ce module n'existe pas dans ce repo.")
            return
        ...
        await self.bot.say("Désinstallé avec succès.")
    ...
    async def _install(self, ctx, repo_name: str, cog: str):
        ...
        if repo_name not in self.repos:
            ...
            return
        if cog not in self.repos[repo_name]:
            await self.bot.say("Ce module n'est pas disponible dans ce repo.")
            return
        ...
        if install_cog:
            await self.bot.say("Installation réussie. Charger maintenant ? (yes/no)")
            answer = await self.bot.wait_for_message(timeout=15,
                                                     author=ctx.message.author)
            if answer is None:
                await self.bot.say("Vous pourrez le faire plus tard avec"
            ...
            await self.bot.say("Module invalide. Installation annulée.")
        else:
            await self.bot.say("Ce module n'existe pas.")
    ...
    async def install(self, repo_name, cog):
        ...
        print("Copie de {}...".format(cog))
        shutil.copy(path, to_path)

        if os.path.exists(cog_data_path):
            print("Copie du fichier {}...".format(cog))
    ...
    def format_patch(repo, cog, log):
        header = "Patch Notes pour %s/%s" % (repo, cog)
        line = "=" * len(header)
        if log:
...
def check_files():
    repos = \
        {'plumx': {'url': "https://github.com/GitAcrown/PlumX.git"}}
