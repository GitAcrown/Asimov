...
#
# Asimov, Bot Discord de Acrown#4424 inspiré de Twentysix26
#
...
description = "Asimov - Bot multifonction de Acrown"
...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    async def get_oauth_url():
        try:
            ...
            return "Impossible de retrouver le lien.Error: {}".format(e)
    ...
    async def set_bot_owner():
        ...
        how_to = "Faîtes `[p]set owner` dans le tchat pour le voir"
        ...
        if bot.user.bot:  # Can fetch owner
            try:
                ...
                return "Impossible de retrouver le propriétaire. " + how_to
        else:
            return "Non reglé. " + how_to
    ...
    async def on_ready():
        ...
        print("{}/{} extensions actives avec {} commandes".format(
            len(bot.cogs), total_cogs, len(bot.commands)))
        print("-----------------")

        if bot.settings.token and not bot.settings.self_bot:
            print("\nUtilisez cet URL pour ramener le bot sur votre serveur:")
            url = await get_oauth_url()
            bot.oauth_url = url
            print(url)

        print("\nServeur officiel: https://discord.me/ekheysn")

        print("Assurez-vous que le bot est constamment à jour en utilisant 'Update' sur le menu.")
...
def interactive_setup(settings):
    ...
    if first_run:
        print("Asimov - Premier lancement\n")
        print("Créez d'abord un compte BOT sur le site de Discord")
        print("et copiez le Token du bot crée.")
    ...
    if not settings.prefixes:
        print("\nChoisissez un préfixe, symbole qui permettra d'appeller le bot:")
        confirmation = False
        while confirmation is False:
            new_prefix = ensure_reply("\nPrefix> ").strip()
            print("\nÊtes-vous sûr de vouloir {0} en tant que préfixe ?\nLes commandes ressembleront à: {0}help"
    ...
    if first_run:
        print("\nEntrez le rôle d'administrateur du serveur.")
        print("Laissez vide pour le nom de base (Admin)")
        settings.default_admin = input("\nAdmin role> ")
        if settings.default_admin == "":
            settings.default_admin = "Admin"
        settings.save_settings()

        print("\nEntrez le rôle de modérateur du serveur.")
        print("Laissez libre pour le nom de base (Modérateur)")
        settings.default_mod = input("\nModerator role> ")
        if settings.default_mod == "":
            settings.default_mod = "Modérateur"
        settings.save_settings()

        print("Configuration terminée. Cette fenêtre passe désormais en 'read-only'.\nGardez cette fenêtre ouverte pour garder le bot connecté.\n"
...
def load_cogs(bot):
    ...
    if owner_cog is None:
        print("Le module Owner n'est pas présent. Veuillez le réinstaller avant de continuer.")
        exit(1)

    if bot.settings._no_cogs:
        bot.logger.debug("Chargement des modules passé (--no-cogs)")
...
if __name__ == '__main__':
    ...
    try:
        ...
        if not bot.settings.no_prompt:
            choice = input("Login indisponible, les serveurs Discord sont peut-être Down.\n> ")
        ...
        bot.logger.exception("Exception fatale, extinction...",
                             exc_info=e)
        loop.run_until_complete(bot.logout())
    finally:
        loop.close()
