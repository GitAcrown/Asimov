...
default_settings = {"BOOST" : 1, "PAYDAY_TIME" : 43200, "PAYDAY_CREDITS" : 150, "SLOT_MIN" : 5, "SLOT_MAX" : 500, "SLOT_TIME" : 200, "JACKPOT" : 100}
...
class Economy:
    ...
    @commands.command(pass_context=True, no_pm=True)
    async def jackpot(self, ctx, offre: int = None):
        """Devenir riche très rapidement c'est possible...

        Vous devez mettre au minimum 2% de votre richesse."""
        author = ctx.message.author
                ...
                if offre >= minimum:
                    if self.bank.can_spend(author, offre):
                        ...
                        if chance == 1:
                            ...
                            self.bank.deposit_credits(author, gain)
                            self.settings[server.id]["JACKPOT"] = 150
                            fileIO("data/economy/settings.json", "save", self.settings)
                        else:
                            self.settings[server.id]["JACKPOT"] += offre
                            await self.bot.say("**Le Jackpot s'élève désormais à {}§**".format(self.settings[server.id]["JACKPOT"]))
                            self.bank.withdraw_credits(author, offre)
                            fileIO("data/economy/settings.json", "save", self.settings)
                    else:
                        await self.bot.say("Tu n'as pas cet argent.")
                else:
                    await self.bot.say("Le minimum pour toi s'élève à {}§.".format(minimum))
            else:
                await self.bot.say("Tu n'as pas de compte bancaire.")
        else:
            await self.bot.say("**Le Jackpot s'élève à {}§.**".format(self.settings[server.id]["JACKPOT"]))
